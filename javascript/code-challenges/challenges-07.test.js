...
const sortStarWarsCharacters = (starWarsArr) => {
  // Solution code here...
  starWarsArr.sort((a, b) => (Number(a.height) < Number(b.height) ? 1 : -1));
  return starWarsArr;
};
...
(recipe.ingredients).map(e=>{
  let indexOfFirst = e.indexOf(' ');
  let indexOfSecond = e.indexOf(' ', (indexOfFirst + 1))
  let name=e.slice(indexOfSecond+1)
  result.push(name)
})
  // for (let i=0; i<arr.length;i++){
  //   result.push(arr.slice(i, i+1))
